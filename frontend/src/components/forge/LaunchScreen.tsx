...
export function LaunchGate({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const hideTimer = window.setTimeout(() => setPhase("hidden"), holdDuration + EXIT_DURATION_MS);
    return () => {
...
function LaunchScreen({
  ...
  return (
    ...
    >
      <button
        type="button"
        className="launch-trigger cursor-pointer rounded-md text-center text-foreground disabled:cursor-default"
        aria-label="Reproduzir animação da marca Forge"
        disabled={!interactive}
        onClick={onActivate}
      >
        <div
          key={activationKey}
          className="launch-brand flex flex-col items-center"
